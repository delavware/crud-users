...
const Modal = ({isShownForm, setIsShownForm, handleSubmit, register, submit, reset, setisUserEditable, isUserEditable, errors}) => {
  ...
  return (
    <div className={`fixed z-[100] top-0 left-0 bg-black/40 bottom-0 right-0 grid place-content-center ${isShownForm ? 'opacity-100 visible' : 'opacity-0 invisible'} onClick={handleCloseModal}`}>
        ...
        <div>
            <input type="text" {...register('first_name')} placeholder='First name *' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full' required/>
        </div>
        <div>
            <input type="text" {...register('last_name')} placeholder='Last name *' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full' required/>
        </div>
        <div>
            <input type="email" {...register('email')} placeholder='Email *' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full' required />
        </div>
        <div>
            <input type="password" {...register('password')} placeholder='Password *' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full' required />
        </div>
        <div>
            <input type="date" {...register('birthday')} placeholder='Birthday' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full'/>
        </div>
        <div>
            <input type="text" {...register('image_url', {
                pattern: {
                    value: /(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|gif|png)/,
                    message: 'Invalid url for images'
                }
            })} placeholder='Image url:' className='px-3 py-1 rounded-md border-[1px]  border-gray-500 w-full'/>
        </div>
